...
HRESULT cLogManager::Init(unsigned int nTarget, HWND hParentWnd, const char * pszFileName)
{
	m_nTarget = nTarget;
...
VOID cLogManager::AddLog(char * strmsg, ...)
{
	char szBuff[1024];
	char szDate[128];		//날짜
	char szTime[128];		//시간
...
VOID cLogManager::LogFile(char * strmsg, ...)
{
	// 현시간을 얻어온다.
	SYSTEMTIME		st;
	GetLocalTime(&st);
...
VOID cLogManager::CreateLogWindow(HWND hWnd)
{
	//
	// 로그 윈도우 생성
	//
	...
	wc.lpfnWndProc = (WNDPROC)cLogManager::WndLogProc;
	wc.cbClsExtra = 0;
	wc.cbWndExtra = 0;
	...
	cx = LOG_WINSIZE_X;
	cy = LOG_WINSIZE_Y;
	x = WINPOS_X + WINSIZE_X;//rcWin.right;
	y = rcWin.top;
...
LRESULT cLogManager::WndLogProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
